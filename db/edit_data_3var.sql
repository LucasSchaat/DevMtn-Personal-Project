...
BEGIN
EXECUTE
    format('UPDATE %I SET outcome = %L, first_category = %L, second_category = %L, third_category = %L WHERE id = $2', 'training_data_' || $1, $3, $4, $5, $6);
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION get_all_training_data()
    RETURNS TABLE (id INTEGER, outcome VARCHAR(250), first_category VARCHAR(250), second_category VARCHAR(250), third_category VARCHAR(250), reference_id INTEGER) AS
    $$
    BEGIN

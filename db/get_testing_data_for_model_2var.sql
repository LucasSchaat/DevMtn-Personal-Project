...
CREATE OR REPLACE FUNCTION get_testing_data_for_model()
    RETURNS TABLE (first_id INTEGER, second_id INTEGER) AS
    $$
    BEGIN
    RETURN QUERY EXECUTE
        format('SELECT first.id as first_id, second.id as second_id FROM %I td JOIN %I first ON td.first_category = first.property JOIN %I second ON td.second_category = second.property', 'testing_data_' || $1, 'first_category_values_' || $1, 'second_category_values_' || $1);
        RETURN;
    END;
$$ LANGUAGE plpgsql;

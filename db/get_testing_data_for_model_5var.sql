...
CREATE OR REPLACE FUNCTION get_testing_data_for_model()
    RETURNS TABLE (first_id INTEGER, second_id INTEGER, third_id INTEGER, fourth_id INTEGER, fifth_id INTEGER) AS
    $$
    BEGIN
    RETURN QUERY EXECUTE
        format('SELECT first.id as first_id, second.id as second_id, third.id as third_id, fourth.id as fourth_id, fifth.id as fifth_id FROM %I td JOIN %I first ON td.first_category = first.property JOIN %I second ON td.second_category = second.property JOIN %I third ON td.third_category = third.property JOIN %I fourth ON td.fourth_category = fourth.property JOIN %I fifth ON td.fifth_category = fifth.property', 'testing_data_' || $1, 'first_category_values_' || $1, 'second_category_values_' || $1, 'third_category_values_' || $1, 'fourth_category_values_' || $1, 'fifth_category_values_' || $1);
        RETURN;
    END;
$$ LANGUAGE plpgsql;
...
-- OLD REFACTORED CODE
-- SELECT first.id as first_id, second.id as second_id, third.id as third_id, fourth.id as fourth_id, fifth.id as fifth_id

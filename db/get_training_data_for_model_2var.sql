...
CREATE OR REPLACE FUNCTION get_training_data_for_model()
    RETURNS TABLE (outcome_id INTEGER, first_id INTEGER, second_id INTEGER) AS
    $$
    BEGIN
    RETURN QUERY EXECUTE
        format('SELECT outcome.id as outcome_id, first.id as first_id, second.id as second_id FROM %I td JOIN %I outcome ON td.outcome = outcome.property JOIN %I first ON td.first_category = first.property JOIN %I second ON td.second_category = second.property', 'training_data_' || $1, 'outcome_values_' || $1, 'first_category_values_' || $1, 'second_category_values_' || $1);
        RETURN;
    END;
$$ LANGUAGE plpgsql;

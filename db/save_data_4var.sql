...
EXECUTE
    format('ALTER TABLE IF EXISTS %I DROP COLUMN IF EXISTS fifth_category, DROP COLUMN IF EXISTS sixth_category', 'training_data_' || $1);
EXECUTE
...
EXECUTE
    format('ALTER TABLE %I ADD COLUMN IF NOT EXISTS second_category VARCHAR(250), ADD COLUMN IF NOT EXISTS third_category VARCHAR(250), ADD COLUMN IF NOT EXISTS fourth_category VARCHAR(250)', 'training_data_' || $1);
EXECUTE
...
CREATE OR REPLACE FUNCTION get_all_training_data()
    RETURNS TABLE (id INTEGER, outcome VARCHAR(250), first_category VARCHAR(250), reference_id INTEGER, second_category VARCHAR(250), third_category VARCHAR(250), fourth_category VARCHAR(250)) AS
    $$
    BEGIN
...
-- -- Incoming data will look like (reference_id, firstOutcome, secondOutcome, outcomeValue, firstCategoryValue, secondCategoryValue, thirdCategoryValue, fourthCategoryValue)
-- -- Ex. (1, good, bad, Yes, brown, 26, green, 5.0)